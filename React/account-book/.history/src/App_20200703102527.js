  ...
  constructor(props) {
    ...
    this.actions = {
      ...
      updateItem: withLoading(async (item, updatedCategoryId) => {
        ...
        const updatedItem = await axios.put(`http://localhost:3004/items/${modifiedItem.id}`, modifiedItem)
        this.setState({
      ...
      /*
        ...
        createItem: (data, categoryId) => {
          ...
          data.monthCategory = `${parsedDate.year}-${padLeft(parsedDate.month)}`
          data.timestamp = new Date(data.date).getTime()
          axios.post('http://localhost:3004/items', { ...data, id: newId, cid: categoryId }).then(newItem => {
            this.setState({
              items: { ...this.state.items, [newId]: newItem.data },
            })
          })
        ...
        updateItem: (item, updatedCategoryId) => {
          ...
          axios.put(`http://localhost:3004/items/${modifiedItem.id}`, modifiedItem).then(updatedItem => {
            this.setState({
              items: { ...this.state.items, [modifiedItem.id]: modifiedItem },
              isLoading: false,
            })
